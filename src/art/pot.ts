...
/** The face's colors: INK and BLUSH, unless the pot's color asks for others. */
type FaceColors = { ink: string; blush: string };

const cheeks = ({ blush }: FaceColors, opacity = 0.85) =>
  `<ellipse cx="420" cy="730" rx="25" ry="15" fill="${blush}" opacity="${opacity}"/>` +
  `<ellipse cx="604" cy="730" rx="25" ry="15" fill="${blush}" opacity="${opacity}"/>`;

const openEyes = ({ ink }: FaceColors) =>
...
const line = ({ ink }: FaceColors, d: string, width = 11) =>
  `<path d="${d}" fill="none" stroke="${ink}" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"/>`;

const smile = (c: FaceColors) => line(c, 'M 488 734 Q 512 758 536 734');
...
const FACES: Record<Mood, (c: FaceColors) => string> = {
  // The logo's face.
  happy: (c) => cheeks(c) + openEyes(c) + smile(c),
  // Eyes closed with pleasure, mouth open.
  joy: (c) =>
    cheeks(c) +
    line(c, 'M 446 706 Q 462 684 478 706') +
    line(c, 'M 546 706 Q 562 684 578 706') +
    `<path d="M 484 728 Q 512 780 540 728 Q 512 736 484 728 Z" fill="${c.ink}"/>` +
    `<path d="M 498 752 Q 512 768 526 752 Q 512 744 498 752 Z" fill="${TONGUE}"/>`,
  // Eyes closed, a small calm mouth.
  sleepy: (c) =>
    cheeks(c, 0.7) +
    line(c, 'M 446 698 Q 462 714 478 698') +
    line(c, 'M 546 698 Q 562 714 578 698') +
    line(c, 'M 502 742 Q 512 750 522 742', 9),
  // Tongue out, and a drop of sweat.
  thirsty: (c) =>
    cheeks(c, 0.6) +
    openEyes(c) +
    `<ellipse cx="512" cy="742" rx="15" ry="12" fill="${c.ink}"/>` +
  ...
  worried: (c) =>
    cheeks(c, 0.6) +
    openEyes(c) +
    line(c, 'M 442 668 L 478 658', 9) +
    line(c, 'M 546 658 L 582 668', 9) +
    line(c, 'M 490 744 Q 501 734 512 744 Q 523 754 534 744', 9),
  // Heart eyes.
  love: (c) => cheeks(c) + heart(462, 700, '#E5484D') + heart(562, 700, '#E5484D') + smile(c),
};

/** The pot's face for a mood, in colors that read on the pot's color. */
export function faceFragment(mood: Mood = 'happy', colorId = 'terracotta'): Fragment {
  const p = potPalette(colorId);
  const colors = { ink: p.ink ?? INK, blush: p.blush ?? BLUSH };
  return { body: (FACES[mood] ?? FACES.happy)(colors) };
}
